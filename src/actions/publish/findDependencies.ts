    ...
    run: async (context: PublishWorkflowContext): Promise<void> => {
        ...
        if (sapDependencies.length > 0) {
            ...
            sapDependencies.forEach(o => {
                o.entries.forEach(e => {
                    if (!context.runtime.manifest.sapEntries[e.tableName]) {
                        context.runtime.manifest.sapEntries[e.tableName] = [];
                    }
                    e.dependency.forEach(d => {
                        if (!context.runtime.manifest.sapEntries[e.tableName].find(c => _.isEqual(c, d.tableKey))) {
                            context.runtime.manifest.sapEntries[e.tableName].push(d.tableKey);
                        }
                    });
                });
            });
