...
export const init: Step<InstallWorkflowContext> = {
    ...
    run: async (context: InstallWorkflowContext): Promise<void> => {
        ...
        //4- check if already installed
        const upgradingPackage = context.rawInput.contextData.systemPackages.find(o => Manifest.compare(o.manifest, new Manifest(context.runtime.package.data.manifest), false));
        if (upgradingPackage) {
            const installVersion = context.runtime.package.data.manifest.version;
            const installedVersion = upgradingPackage.manifest.get().version;
            if (eq(installVersion, installedVersion)) {
                if (context.rawInput.packageData.overwrite) {
                    if (upgradingPackage.isDirty()) {
